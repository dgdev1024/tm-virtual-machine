...
- **Long**: A group of 32 bits (a 32-bit integer, between 0 and 4294967295).
...
- **XRAM**: Executable RAM, a space in RAM intended to be used for executable code.
...
| `0x00003000`  | `0x7FFFFFFF`  | Reserved for the program code.                                   |
| `0x80000000`  | `0xFFFCFFFF`  | Reserved for RAM (Random Access Memory)                          |
...
| `0xFFFF0000`  | `0xFFFFFFFF`  | Reserved for Quick RAM (QRAM)                                    |
...
of the stack. The stacks are implemented in the RAM region at the following addresses:
...
| `DSP`    | Word   | `0xFFFF`      | Data Stack Pointer, points to the top of the data stack.              |
| `CSP`    | Word   | `0xFFFF`      | Call Stack Pointer, points to the top of the call stack.              |
| `F`      | Byte   | `0x00`        | Flags Register, stores status information such as the carry flag.     |
| `IE`     | Byte   | `0x00`        | Interrupt Enable, enables or disables interrupts.                     |
| `IF`     | Byte   | `0x00`        | Interrupt Flag, indicates which interrupts are pending.               |
...
|------|--------|-------------------|-----------------------------------------------------------------------|
| 0    | `Z`    | Zero Flag         | Set if the result of the last operation was zero.                     |
| 1    | `N`    | Subtraction Flag  | Set if the last operation was a subtraction.                          |
| 2    | `H`    | Half-Carry Flag   | Set if there was a carry from the lower nibble in the last operation. |
| 3    | `C`    | Carry Flag        | Set if there was a carry from the last operation.                     |
| 4    | `S`    | Sign Flag         | Set if the result of the last operation was negative.                 |
| 5    | `P`    | Parity Flag       | Set if the result of the last operation has an even number of bits.   |
| 6    | `L`    | Halt Flag         | Set if the CPU is halted.                                             |
| 7    | `T`    | Stop Flag         | Set if the CPU is stopped.                                            |

- The `L` flag is set when the CPU is halted.
    - The CPU will not execute any instructions while the `L` flag is set.
    - The `L` flag can be set by the `HALT` instruction.
    - The `L` flag is cleared automatically when an interrupt is requested.
- The `T` flag is set when the CPU is stopped.
    - The CPU will not operate while the `T` flag is set.
    - The `T` flag can be set by the `STOP` instruction, or if an exception occurs.
    - The `T` flag is intended to signal program termination or an unrecoverable error.
    - The `T` flag can only be cleared by resetting the CPU.
    ...
    - A `RET` or `RETI` instruction while the call stack is empty will set the `T` flag, and set
        the `EC` register to `0x00`.
...
| `0x03` | `TM_EC_BAD_READ`                 | Attempt to read non-readable memory.                                  |
| `0x04` | `TM_EC_BAD_WRITE`                | Attempt to write to read-only memory.                                 |
| `0x05` | `TM_EC_BAD_EXECUTE`              | Attempt to execute non-executable memory.                             |
| `0x06` | `TM_EC_DATA_STACK_OVERFLOW`      | Data stack overflow.                                                  |
| `0x07` | `TM_EC_DATA_STACK_UNDERFLOW`     | Data stack underflow.                                                 |
| `0x08` | `TM_EC_CALL_STACK_OVERFLOW`      | Call stack overflow.                                                  |
| `0x09` | `TM_EC_HARDWARE`                 | Hardware error.                                                       |
...
exception, or to the address of the memory access which caused the exception.
...
| `OS`      | Overflow Set: The instruction is executed if the overflow flag is set.               |
| `OC`      | Overflow Clear: The instruction is executed if the overflow flag is clear.           |
| `NS`      | Negative Set: The instruction is executed if the sign flag is set.                   |
| `NC`      | Negative Clear: The instruction is executed if the sign flag is clear.               |
| `PS`      | Parity Set: The instruction is executed if the parity flag is set.                   |
| `PC`      | Parity Clear: The instruction is executed if the parity flag is clear.               |
...
|-----------------|-------------|----------|------------------------------------------------------------------------------|
| `0x0000`        | `NOP`       | `000000` | No Operation: Do nothing.                                                    |
| `0x0100`        | `STOP`      | `000000` | Stop CPU: Stop the CPU.                                                      |
| `0x0200`        | `HALT`      | `000000` | Halt CPU: Halt the CPU.                                                      |
| `0x03XY`        | `SEC XY`    | `000000` | Set Error Code: Set the error code to the immediate value `XY`.              |
| `0x0400`        | `CEC`       | `000000` | Clear Error Code: Clear the error code.                                      |
| `0x0500`        | `DI`        | `000000` | Disable Interrupts: Disable interrupts.                                      |
| `0x0600`        | `EI`        | `000000` | Enable Interrupts: Enable interrupts.                                        |
| `0x0700`        | `DAA`       | `?0-???` | Decimal Adjust Accumulator: Adjust the accumulator for BCD arithmetic.       |
| `0x08X0`        | `CPA X`     | `0++00?` | Compliment Accumulator: Compliment accumulator register `X`.                 |
| `0x09X0`        | `SWAP X`    | `?---00` | Swap the high and low halves of register `X`.                                |
| `0x0A00`        | `SCF`       | `0--1-0` | Set Carry Flag: Set the carry flag.                                          |
| `0x0B00`        | `CCF`       | `0--?-0` | Compliment Carry Flag: Compliment the carry flag.                            |
...
- `DAA`:
    ...
    - `P`: Set if the result has an even number of bits.
- `CPA`:
    - `P`: Set if the result has an even number of bits.
- `SWAP`:
    - `Z`: Set if the result is zero.
...
| `0x14X0`        | `LDH X, [A8]`   | Load from Hardware: Load the value at address `A8` into register `X`.                                 |
| `0x15XY`        | `LDI X, [Y]`    | Load then Increment: Load the value at address in register `Y` into `X`, then increment `Y`.          |
| `0x16XY`        | `LDD X, [Y]`    | Load then Decrement: Load the value at address in register `Y` into `X`, then decrement `Y`.          |
...
| `0x1AX0`        | `STH [A8], X`   | Store to Hardware: Store the value in register `X` at address `A8`.                                   |
| `0x1BXY`        | `STI [X], Y`    | Store then Increment: Store the value in register `Y` at address in register `X`, then increment `X`. |
| `0x1CXY`        | `STD [X], Y`    | Store then Decrement: Store the value in register `Y` at address in register `X`, then decrement `X`. |
...
| Opcode          | Mnemonic     | Flags    | Description                                                                                                           |
|-----------------|--------------|----------|-----------------------------------------------------------------------------------------------------------------------|
| `0x30X0`        | `INC X`      | `?-?00?` | Increment: Increment register `X`.                                                                                    |
| `0x310Y`        | `INC [Y]`    | `?-?00?` | Increment Memory: Increment the value at address in register `Y`.                                                     |
| `0x32X0`        | `DEC X`      | `?+?00?` | Decrement: Decrement register `X`.                                                                                    |
| `0x330Y`        | `DEC [Y]`    | `?+?00?` | Decrement Memory: Decrement the value at address in register `Y`.                                                     |
| `0x34X0`        | `ADD X, I`   | `?-??0?` | Add Immediate: Add the immediate value `I` to accumulator register `X`.                                               |
| `0x35XY`        | `ADD X, Y`   | `?-??0?` | Add Register: Add register `Y` to accumulator register `X`.                                                           |
| `0x36XY`        | `ADD X, [Y]` | `?-??0?` | Add Memory: Add the value at address in register `Y` to accumulator register `X`.                                     |
| `0x37X0`        | `ADC X, I`   | `?-??0?` | Add with Carry Immediate: Add the immediate value `I` to accumulator register `X` with carry.                         |
| `0x38XY`        | `ADC X, Y`   | `?-??0?` | Add with Carry Register: Add register `Y` to accumulator register `X` with carry.                                     |
| `0x39XY`        | `ADC X, [Y]` | `?-??0?` | Add with Carry Memory: Add the value at address in register `Y` to accumulator register `X`.                          |
...
| Opcode          | Mnemonic     | Flags    | Description                                                                                           |
|-----------------|--------------|----------|-------------------------------------------------------------------------------------------------------|
| `0x40X0`        | `AND X, I`   | `?-+-0?` | Bitwise AND Immediate: Bitwise AND the immediate value `I` with accumulator register `X`.             |
| `0x41XY`        | `AND X, Y`   | `?-+-0?` | Bitwise AND Register: Bitwise AND register `Y` with accumulator register `X`.                         |
| `0x42XY`        | `AND X, [Y]` | `?-+-0?` | Bitwise AND Memory: Bitwise AND the value at address in register `Y` with accumulator register `X`.   |
| `0x43X0`        | `OR X, I`    | `?---0?` | Bitwise OR Immediate: Bitwise OR the immediate value `I` with accumulator register `X`.               |
| `0x44XY`        | `OR X, Y`    | `?---0?` | Bitwise OR Register: Bitwise OR register `Y` with accumulator register `X`.                           |
| `0x45XY`        | `OR X, [Y]`  | `?---0?` | Bitwise OR Memory: Bitwise OR the value at address in register `Y` with accumulator register `X`.     |
| `0x46X0`        | `XOR X, I`   | `?---0?` | Bitwise XOR Immediate: Bitwise XOR the immediate value `I` with accumulator register `X`.             |
| `0x47XY`        | `XOR X, Y`   | `?---0?` | Bitwise XOR Register: Bitwise XOR register `Y` with accumulator register `X`.                         |
| `0x48XY`        | `XOR X, [Y]` | `?---0?` | Bitwise XOR Memory: Bitwise XOR the value at address in register `Y` with accumulator register `X`.   |
...
| Opcode          | Mnemonic     | Flags    | Description                                                                                                               |
|-----------------|--------------|----------|---------------------------------------------------------------------------------------------------------------------------|
| `0x60X0`        | `SLA X`      | `?--?0?` | Shift Left Arithmetic: Shift the bits in accumulator register `X` left, shifting in zeros.                                |
| `0x610Y`        | `SLA [Y]`    | `?--?0?` | Shift Left Arithmetic Memory: Shift the bits at address in register `Y` left, shifting in zeros.                          |
| `0x62X0`        | `SRA X`      | `?--?0?` | Shift Right Arithmetic: Shift the bits in accumulator register `X` right, leaving the old bit 7 unchanged.                |
| `0x630Y`        | `SRA [Y]`    | `?--?0?` | Shift Right Arithmetic Memory: Shift the bits at address in register `Y` right, leaving the old bit 7 unchanged.          |
| `0x64X0`        | `SRL X`      | `?--?0?` | Shift Right Logical: Shift the bits in accumulator register `X` right, shifting in zeros.                                 |
| `0x650Y`        | `SRL [Y]`    | `?--?0?` | Shift Right Logical Memory: Shift the bits at address in register `Y` right, shifting in zeros.                           |
| `0x66X0`        | `RL X`       | `?--?0?` | Rotate Left Through Carry: Rotate the bits in accumulator register `X` left through the carry flag.                       |
| `0x670Y`        | `RL [Y]`     | `?--?0?` | Rotate Left Through Carry Memory: Rotate the bits at address in register `Y` left through the carry flag.                 |
| `0x68X0`        | `RLC X`      | `?--?0?` | Rotate Left: Rotate the bits in accumulator register `X` left, shifting in the carry flag.                                |
| `0x690Y`        | `RLC [Y]`    | `?--?0?` | Rotate Left Memory: Rotate the bits at address in register `Y` left, shifting in the carry flag.                          |
| `0x6AX0`        | `RR X`       | `?--?0?` | Rotate Right Through Carry: Rotate the bits in accumulator register `X` right through the carry flag.                     |
| `0x6B0Y`        | `RR [Y]`     | `?--?0?` | Rotate Right Through Carry Memory: Rotate the bits at address in register `Y` right through the carry flag.               |
| `0x6CX0`        | `RRC X`      | `?--?0?` | Rotate Right: Rotate the bits in accumulator register `X` right, shifting in the carry flag.                              |
| `0x6D0Y`        | `RRC [Y]`    | `?--?0?` | Rotate Right Memory: Rotate the bits at address in register `Y` right, shifting in the carry flag.                        |
...
| Opcode          | Mnemonic     | Flags    | Description                                                                                           |
|-----------------|--------------|----------|-------------------------------------------------------------------------------------------------------|
| `0x700Y`        | `BIT I, Y`   | `?-+000` | Bit Test: Test bit `I` of register `Y`. `I` is an immedate value.                                     |
| `0x710Y`        | `BIT I, [Y]` | `?-+000` | Bit Test Memory: Test bit `I` of the value at address in register `Y`. `I` is an immedate value.      |
| `0x72X0`        | `SET I, X`   | `00000?` | Bit Set: Set bit `I` of register `X`. `I` is an immedate value.                                       |
| `0x730Y`        | `SET I, [Y]` | `00000?` | Bit Set Memory: Set bit `I` of the value at address in register `Y`. `I` is an immedate value.        |
| `0x74X0`        | `RES I, X`   | `00000?` | Bit Reset: Reset bit `I` of register `X`. `I` is an immedate value.                                   |
| `0x750Y`        | `RES I, [Y]` | `00000?` | Bit Reset Memory: Reset bit `I` of the value at address in register `Y`. `I` is an immedate value.    |
